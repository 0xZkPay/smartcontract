...
export class LoyaltyNFTCreated extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save LoyaltyNFTCreated entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type LoyaltyNFTCreated must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("LoyaltyNFTCreated", id.toBytes().toHexString(), this);
    }
  }

  static load(id: Bytes): LoyaltyNFTCreated | null {
    return changetype<LoyaltyNFTCreated | null>(
      store.get("LoyaltyNFTCreated", id.toHexString())
    );
  }
